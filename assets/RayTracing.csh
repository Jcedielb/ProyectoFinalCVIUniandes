...
END_SHADER_DECLARATION(CSMain, 8, 8)
{
    ...
    float  NdotL       = max(0.0, dot(LightDir, WNormal));
     ...
     if (NdotL > 0.0)
     {
        // Calculamos origen del rayo de sombra
        float3 shadowOrigin = WPos.xyz + WNormal.xyz * SMALL_OFFSET * DisToCamera;

        // Si estamos sombreando el suelo (normal casi vertical), desplazamos el origen
        // en XZ según MoveDir y FlowAmp para simular flujo gelatinoso:
        if (abs(WNormal.y - 1.0) < 0.01)
        {
            shadowOrigin.xz -= g_Constants.MoveDir.xz * g_Constants.FlowAmp;
        }

        NdotL *= CastShadow(shadowOrigin,
                            LightDir,
...
// Reflection
{
    ...
    // 1) Calculamos la reflexión normal
    float3 reflDir = reflect(ViewRayDir, WNormal);

    // 2) Si estamos en el suelo (normal casi vertical), aplicamos el estiramiento gelatinoso
if (abs(WNormal.y - 1.0) < 0.01)
{
    // Factor de atenuación para no pasarse
    float atten = 0.1; 
    // opcionalmente: float atten = saturate(1.0 - WNormal.y);
    reflDir.xz -= g_Constants.MoveDir.xz * g_Constants.FlowAmp * atten;
    reflDir = normalize(reflDir);
}


    // 3) Asignamos al atributo
    Attribs.ReflectionRayDir       = reflDir;
