...
#include "BasicMath.hpp" // te da clamp(x, lo, hi)
...
void Tutorial22_HybridRendering::CreateSceneObjects(const uint2 CubeMaterialRange, const Uint32 GroundMaterial)
{
    ...
    const auto AddCubeObject = [&](float Angle, float X, float Y, float Z, float Scale, bool IsDynamic = false) //
    {
        ...
        if (IsDynamic)
        {
            ...
            // Asigna un slowdown único: por ejemplo, 0.8 + 0.05 por cada dinámico ya existente
    ...
    AddCubeObject(0.f, 0.f, 0.f,  10.f, 1.1f, true);
        ...
        // Generar copias “gelatinosas” de cada objeto dinámico
    // Guardar cuántos dinámicos había originalmente
    const size_t OrigDynCount = m_Scene.DynamicObjects.size();
    for (size_t i = 0; i < OrigDynCount; ++i)
    {
        // Índice y datos del objeto base
        ...
        {
            // Añadir un nuevo HLSL::ObjectAttribs idéntico
...
void Tutorial22_HybridRendering::Initialize(const SampleInitInfo& InitInfo)
{
    ...
    // Setup camera.
    // Nueva ubicación: más lejos y un poco más baja
    m_Camera.SetPos(float3{-20.0f, 2.0f, -12.0f});
    // Baja el ángulo de pitch para que mire más hacia arriba desde abajo
...
void Tutorial22_HybridRendering::Render()
{
    // Update constants
    {
        ...
        // ** NUEVO: dirección de movimiento para deformar gelatinosidad horizontal **
        // ** NUEVO: dirección de movimiento suavizada para deformar el gel **
            GConst.MoveDir = float4{m_SmoothedMoveDir.x,
                                     m_SmoothedMoveDir.y,
                                     m_SmoothedMoveDir.z,
                                     0.f};
        // amplitud del estirón horizontal
        ...
        m_pImmediateContext->UpdateBuffer(
            m_Constants,
            0,
            static_cast<Uint32>(sizeof(GConst)),
            &GConst,
            RESOURCE_STATE_TRANSITION_MODE_TRANSITION);

        // Actualiza también el buffer de ObjectAttribs
        m_pImmediateContext->UpdateBuffer(
...
void Tutorial22_HybridRendering::Update(double CurrTime, double ElapsedTime)
{
    ...
    if (m_PlayerPosition.y < m_GroundHeight)
    {
        ...
        if ((m_PlayerVelocityY) < 0.1f) // Si la velocidad del rebote es muy pequeña, detener el rebote
        {
            m_PlayerVelocityY = 0.0f;
        }
    }

    // --- Cálculo del movimiento gelatinoso ---
    ...
    // --- Actualiza cámara y límites (igual que antes) ---
    ...
    // --- Rebote "vertical" + deformación "gelatinoso" ---
    for (auto& DynObj : m_Scene.DynamicObjects)
    {
        ...
        // Si la desintegración está activada, comenzamos a modificar la matriz
        if (m_Desintegrating)
        {
            // Aumentamos el tiempo transcurrido desde el inicio de la desintegración
            ...
            newModelMat                  = desintegrationXform * newModelMat; // Aplica la transformación de desintegración
        }
    ...
    // --- Finalización de la desintegración ---
    if (m_ElapsedTimeSinceDesintegration > 3.0f) // 3 segundos de desintegración
    {
        m_Desintegrating                 = false; // Detenemos la desintegración después de 3 segundos
...
void Tutorial22_HybridRendering::UpdateUI()
{
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        // Control de la amplitud de gelatinosidad
        ImGui::SliderFloat("Flow Amp", &m_FlowAmplitude, 0.0f, 1.0f, "%.2f");

        // Nuevo: Control de la gravedad
        ImGui::SliderFloat("Gravity", &m_Gravity, 0.0f, 10.0f, "%.2f");

        // Nuevo: Control de la dirección del movimiento
        if (ImGui::SliderFloat("Move Direction X", &m_MoveDir.x, -1.0f, 1.0f, "%.2f"))
        {
            m_MoveDir = normalize(m_MoveDir);
        }
        if (ImGui::SliderFloat("Move Direction Z", &m_MoveDir.z, -1.0f, 1.0f, "%.2f"))
        {
            m_MoveDir = normalize(m_MoveDir);
        }
        ...
        // Control para el "Landing Squash" (cuánto se aplasta al aterrizar)
        ImGui::SliderFloat("Landing Squash", &m_LandingSquash, 0.0f, 1.0f, "%.2f");

        // Control para la suavización de la dirección de movimiento
        ImGui::SliderFloat("Movement Direction Smoothness", &m_MoveSmoothSpeed, 0.0f, 10.0f, "%.2f");

        // Nuevo: Control de la dirección de la cámara
                ...
                if (ImGui::Button("Desintegrar Cubo"))
        {
            ...
            m_ElapsedTimeSinceDesintegration = 0.0f; // Reinicia el tiempo de desintegración
        }
    }
    ImGui::End();
}
